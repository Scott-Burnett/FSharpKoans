...
module ``14: List operations are so easy, you could make them yourself!`` =
    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int = // write a function to find the length of a list
            let rec inner l length =
                match l with
                |[] -> length
                |h::t -> inner t (length + 1)
            inner xs 0
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list = // write a function which adds 1 to each element
            let rec inner l r =
                match l with 
                |[] -> r
                |h::t -> inner t (r @ [h + 1])
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list = // write a function which doubles each element
            let rec inner l r =
                    match l with 
                    |[] -> r
                    |h::t -> inner t (r @ [h * 2])
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list = // write a map which applies f to each element
            let rec inner l r =
                    match l with 
                    |[] -> r
                    |h::t -> inner t (r @ [f h])
