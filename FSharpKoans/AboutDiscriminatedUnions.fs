...
module ``08: The Good Kind of Discrimination`` = 
    ...
    let ``04 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let d = Node ("Four", Empty, Empty) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
        let c = Node ("Three", d, Empty)
        let b = Node ("Two", c, Empty)
        let a = Node ("One", b, Empty)
